...
N=nrow(Knee) # NUMBER OF SUBJECTS

J=ncol(Knee) # NUMBER OF MAXIMUM OBSERVATIONS PER SUBJECTS
...
for (iter_out in case) { # HERE WE CAN SELECTED WHICH SCENARIO(S) SIMULATE 
  ...
  for (iter in 1:R) {
    ...
    X_s[i,1:M[i]]=Knee[i,1:M[i]]
    X_se[i,1:M[i]]=Knee[i,1:M[i]]+rnorm(1,0,10) # WE ADD NOISE
    }
    ...
    for (i in 1:N) {
      ...
      Beta[i,1:(M[i]),2]=(exp(-2*((t[1:M[i]]-0.25)/M[i])) - exp(-((t[1:M[i]]-0.75)/M[i])) )/10#(exp(-2*((t[1:M[i]]-0.75))) - exp(-2*((t[1:M[i]]-0.5))))/10
