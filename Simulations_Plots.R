...
aux=Y_values[which(Y_values<quantile(Y_values,probs = 0.95))] #quantile(Y_values,probs = 0.95)
# aux=Y_values_2[which(Y_values_2<quantile(Y_values_2,probs = 0.95))] #quantile(Y_values,probs = 0.95)
Y_ERRORES_DF=data.frame(values=aux,method=as.factor(c(rep("VDFR",R-4),rep("SOF",R-10),rep("FF-VDFR",R-1)))) # POISSON UNIFORM R-7,R,R Y_1 # POISSON NegBin R-12,R-1,R-4 Y_1 # POISSON NegBin R-7,R-2,R-2 Y_2
...
aux=B_values_2[which(B_values_2<quantile(B_values_2,probs = 0.95))] #quantile(B_values_2,probs = 0.95)
