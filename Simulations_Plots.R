...
aux=Y_values_2[which(Y_values_2<3)] #quantile(Y_values_2,probs = 0.95)
Y_ERRORES_DF=data.frame(values=aux,method=as.factor(c(rep("VDFR",R),rep("SOF",R-1),rep("FF-VDFR",R)))) # POISSON UNIFORM R-7,R,R Y_1 # POISSON NegBin R-12,R-1,R-4 Y_1 # POISSON NegBin R-7,R-2,R-2 Y_2
...
aux=B_values_2[which(B_values_2<0.08)] #quantile(B_values_2,probs = 0.95)
B_ERRORES_DF=data.frame(values=aux,method=as.factor(c(rep("FF-VDFR",R-1),rep("VDFR",R-4)))) # POISSON NegBin R-6,R-4 B_2 # POISSON NegBin round3 R-1,R-4 B_2


B_ERRORES_DF=data.frame(values=B_values_1,method=as.factor(c(rep("FF-VDFR",R),rep("VDFR",R))))
